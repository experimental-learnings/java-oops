...
public class Board{
	...
	private int SIZE;
	   ...
	   public boolean checkRows(Seed theSeed){
		   ...
		   for(int i=0;i<SIZE;i++){
			   if(cells[currentRow][i].content == theSeed){count=count+1;}
		   }
		   return count==SIZE;
	   }
	   public boolean checkCols(Seed theSeed){
		   int count = 0;
		   for(int i=0;i<SIZE;i++){
			   if(cells[i][currentCol].content == theSeed){count=count+1;}
		   }
		   return count==SIZE;
	   }
	   public boolean checkDiagonals(Seed theSeed){
		   ...
		   if(currentRow == currentCol){
			   for(int i=0;i<SIZE;i++){
				   if(cells[i][i].content == theSeed){count=count+1;}
			   }
			   return count==SIZE;
		   }
		   else if(currentRow + currentCol == SIZE-1){
			   for(int i=0;i<SIZE;i++){
				   if(cells[i][SIZE-i-1].content == theSeed){count1=count1+1;}
			   }
			   return count1==SIZE;
		   }
		   return count==SIZE;
	...
	public boolean moveValidate(int row, int col, Seed theSeed) {
		if (row >= 0 && row < ROWS && col >= 0 && col < COLS && cells[row][col].content == Seed.EMPTY) {
